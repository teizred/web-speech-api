...
const CATEGORIES = [
  ...
  {
    label: "🥤 Boissons",
    products: ["Eau Plate", "Eau Pétillante", "Oasis Tropical", "Green Apple Sprite", "Coca-Cola Sans-Sucres", "Coca-Cola Cherry Zéro", "Coca-Cola", "Sprite Sans-Sucres", "Fanta Sans-Sucres", "Minute Maid Orange", "Lipton Ice Tea", "P'tit Nectar Pomme", "Capri-Sun Tropical", "Americano Glacé", "Café Latte Glacé Gourmand", "Café Latte Glacé", "Thé Glacé Pêche", "Délifrapp Cookie", "Délifrapp Vanille", "Smoothie Mangue Papaye", "Smoothie Banane Fraise", "Jus d'Orange", "Jus de Pomme"],
  },
  {
    label: "☕ McCafé",
      ...
      {CATEGORIES.map((group) => (
        <div key={group.label}>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {group.products.map((productName) => {
              ...
              if (lowerCat.includes("mccafé") && !["Espresso", "Ristretto", "Double Espresso"].includes(productName)) {
                defaultSize = "Grand";
              }

              const key = `${productName}__${defaultSize || "null"}`;
              ...
              return (
                ...
                >
                  <div className="flex-1 flex flex-col justify-center min-h-[40px]">
                    ...
                    {defaultSize && (
                      <span className="text-[10px] text-slate-400 font-medium">{defaultSize}</span>
                    )}
                  </div>

                  <div className="flex items-center gap-3 bg-slate-50 rounded-lg p-1 w-full justify-between">
                    <button
                      onClick={() => handleQuantityChange(productName, defaultSize, currentLoss, -1)}
